        ...
        {loading ? (
          ...
          <>
            ...
            <ul className="divide-y divide-gray-100 dark:divide-gray-700 sm:hidden">
              {filtered.map((comp) => (
                <li key={comp.id} className="p-4 space-y-2">
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-2 min-w-0">
                      <input type="checkbox" checked={selected.has(comp.id)} onChange={() => toggleSelect(comp.id)} className="rounded border-gray-300 text-teal-600 focus:ring-teal-500 shrink-0" />
                      <span className="text-sm font-medium text-gray-900 dark:text-white">{comp.nome}</span>
                    </div>
                    <span className={`shrink-0 inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${comp.ativo ? 'bg-green-100 text-green-800 dark:bg-green-900/40 dark:text-green-300' : 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400'}`}>
