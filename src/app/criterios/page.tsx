              ...
              {criterios.map((c) => (
                <li key={c.id} className="p-4 space-y-2">
                  ...
                  <div className="flex items-center justify-end gap-2">
                    <ListActionButton action="edit" label={`Editar ${c.nome} (${c.area.nome})`} onClick={() => openEdit(c)} />
                    <ListActionButton action={c.ativo ? 'deactivate' : 'activate'} label={`${c.ativo ? 'Desativar' : 'Ativar'} ${c.nome} (${c.area.nome})`} onClick={() => handleToggleAtivo(c)} />
                    <ListActionButton action="delete" label={`Excluir ${c.nome} (${c.area.nome})`} onClick={() => setDeleteConfirm(c.id)} />
                  </div>
                </li>
              ))}
            </ul>
